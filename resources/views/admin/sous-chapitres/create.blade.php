<x-app-layout>
    ...
    <div class="py-8">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            ...
            <!-- AI Generation -->
            <div class="bg-gradient-to-br from-purple-50 via-indigo-50 to-cyan-50 rounded-2xl p-6 shadow-sm border border-purple-100" x-data="aiCoursGenerator()">
                <div class="flex items-center gap-3 mb-4">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-500">Décrivez le thème et l'IA génère le titre et le contenu pédagogique.</p>
                    </div>
                </div>

                <div class="space-y-4">
                    <div>
                        <x-input-label value="Thème du cours" />
                        <x-text-input x-model="sujet" type="text" class="mt-1 block w-full" placeholder="Ex: Les balises sémantiques HTML5, Les boucles en Python..." />
                    </div>
                ...
                function aiCoursGenerator() {
                    return {
                        ...
                        async generate() {
                            ...
                            this.successMsg = '';
                            try {
                                const response = await fetch('{{ route("admin.ai.generate-cours") }}', {
                                    method: 'POST',
                                    headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}', 'Accept': 'application/json' },
                                    body: JSON.stringify({
                                        formation_nom: @js($chapitre->formation->nom),
                                        chapitre_titre: @js($chapitre->titre),
                                        sujet: this.sujet
                                    })
                                });
                                ...
                                if (data.success) {
                                    ...
                                    document.getElementById('contenu').value = data.data.contenu;
                                    this.successMsg = 'Cours généré ! Vous pouvez modifier le contenu avant de valider.';
                                } else {
