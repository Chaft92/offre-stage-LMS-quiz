                ...
                <div class="divide-y divide-gray-50">
                            ...
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-gray-500 text-sm">
                                        {{ $quiz->sousChapitre->chapitre->formation->nom ?? '' }} &middot; {{ $quiz->sousChapitre->titre ?? '' }}
                                        &middot; {{ $quiz->questions_count }} question(s)
                                    </p>
                                </div>
                            </div>
                            <div class="flex items-center gap-2">
                                <span class="inline-flex px-2.5 py-1 text-xs font-medium rounded-lg {{ $quiz->published ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' : 'bg-amber-50 text-amber-700 border border-amber-100' }}">
                                    {{ $quiz->published ? 'Publi&eacute;' : 'Brouillon' }}
                                </span>
                                <a href="{{ route('admin.quizzes.results', $quiz) }}" class="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition" title="R&eacute;sultats">
                                ...
                                <form action="{{ route('admin.quizzes.publish', $quiz) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="p-2 text-gray-400 hover:text-{{ $quiz->published ? 'amber' : 'emerald' }}-600 hover:bg-{{ $quiz->published ? 'amber' : 'emerald' }}-50 rounded-lg transition" title="{{ $quiz->published ? 'D&eacute;publier' : 'Publier' }}">
                                        @if($quiz->published)
                    ...
                    @empty
                        <div class="p-12 text-center">
                            ...
                            <p class="text-gray-500">Aucun quiz. <a href="{{ route('admin.quizzes.create') }}" class="text-indigo-600 hover:underline font-medium">Cr&eacute;er le premier</a>.</p>
                        </div>
                    @endforelse
                </div>
            </div>
